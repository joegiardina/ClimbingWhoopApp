...
const WorkoutComponent: React.FC<{route:any, navigation:any}> = ({route, navigation}) => {
  ...
  return (
    <ScrollView
      ...
      <Text style={{margin: spacing.normal,  fontSize: fontSizes.medium, color: textColor}}>Duration: {min !== max ? `${min} to ${max} minutes` : `${min} minutes`}</Text>
      ...
      <TouchableOpacity
        onPress={() => navigation.navigate('WorkoutScreen', {...route.params, result: {name, result}})}
        style={{
